...
import type { AnalyzedGame, Game } from "@/types/Game";
...
function RouteComponent() {
  ...
  const query = useQuery({
    ...
    enabled: !!user,
    select: (res) => "data" in res.data && res.data.data,
  });
      ...
      {query.isError ? (
        ...
        <div className="md:grid md:grid-cols-3">
          {query.data.map((game: AnalyzedGame) => (
            <Link to="/games/$detail" params={{ detail: game.id }}>
              <Card
              ...
              >
                <p className="text-red-400 flex flex-row gap-2 items-center">
                  Black Accuracy:
                  <p className="font-bold text-md">
                    {Math.floor(game.accuracies.black)}%
                  </p>
                </p>
                <p className="text-green-400 flex flex-row gap-2 items-center">
                  White Accuracy:
                  <p className="font-bold text-md">
